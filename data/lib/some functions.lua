...
function getClockString(tw)
	local a = getWorldTime()
	local b = a / 60
	local horas = math.floor(b)
	local minut = a - (60 * horas)
...
function doReturnPokemon(cid, pokemon, pokeball, effect, hideeffects, blockevo)
	...
	if blockevo then
		edit = false
		doPlayerSendCancel(cid, "Tu Pokémon no pudo evolucionar por un error del servidor; espera mientras lo solucionamos.")
	end
...
-----------------------------------------------------------------
	if item.uid ~= getPlayerSlotItem(cid, CONST_SLOT_FEET).uid then
		doPlayerSendCancel(cid, "¡Debes colocar tu Poké Ball en el lugar correcto!")
	return TRUE
	end
	...
	if thishp <= 0 then
		if isInArray(pokeballs[btype].all, item.itemid) then
			...
			doPlayerSendCancel(cid, "Este Pokémon está debilitado.")
		    return true
		end
	end
   ...
   if #balls >= 1 then
      for _, uid in ipairs(balls) do
          ...
          if not isInArray(lowPokes, pokemon) and nome == pokemon then
             return doPlayerSendTextMessage(cid, 27, "No puedes llevar dos Pokémon iguales.")
          else
...
if #low >= 3 then
   return doPlayerSendTextMessage(cid, 27, "No puedes llevar más de tres Pokémon iguales de bajo nivel.")
end   
end
---------------------------------------------------------------------------------------------------------------------------------------------------
	...
	doPlayerSendCancel(cid, "Necesitas nivel "..(x.level + boosts).." para usar este Pokémon.")
	return true
	end
	...
	if shinysClan[pokemon] and (getPlayerGroupId(cid) < 4 or getPlayerGroupId(cid) > 6) then --alterado v1.9
	   if getPlayerStorageValue(cid, 86228) ~= shinysClan[pokemon][1] then
	      doPlayerSendCancel(cid, "Debes ser miembro del clan "..shinysClan[pokemon][2].." para usar este Pokémon.")
	      return true   
       elseif getPlayerStorageValue(cid, 862281) ~= 5 then
          doPlayerSendCancel(cid, "Debes tener al menos rango 5 para usar este Pokémon.")
	      return true
       end
    end
    --------------------------------------------------------------------------------------
