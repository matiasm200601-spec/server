...
local t = "normal = (.-), great = (.-), super = (.-), ultra = (.-), saffari = (.-);"
local msg = {}
table.insert(msg, "Tienes wasted: ")
...
local happy = 250
	...
	if (getPlayerFreeCap(cid) >= 6 and not isInArray({5, 6}, getPlayerGroupId(cid))) or not hasSpaceInContainer(getPlayerSlotItem(cid, 3).uid) then   
      doPlayerSendMailByName(getCreatureName(cid), item, 1)   
		doPlayerSendTextMessage(cid, 27, "¡Felicidades, capturaste un Pokémon ("..poke..")!")
		doPlayerSendTextMessage(cid, 27, "Como ya tienes seis Pokémon, esta Poké Ball fue enviada a tu depósito.")     
    else
		doPlayerSendTextMessage(cid, 27, "¡Felicidades, capturaste un ("..poke..")!")
    end
...
function getPlayerInfoAboutPokemon(cid, poke)
	...
	if not a then
		print("Error while executing function \"getPlayerInfoAboutPokemon(\""..getCreatureName(cid)..", "..poke..")\", "..poke.." no existe.")
	return false
	end
