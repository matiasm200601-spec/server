function onSay(cid, words, param, channel)
	...
	if(account == 0) then
		...
		if(account == 0) then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "El jugador o la cuenta '" .. param .. "' no existe.")
			return true
		end
	...
	if(isIpBanished(tmp) and doRemoveIpBanishment(tmp)) then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "El bloqueo de IP para " .. doConvertIntegerToIp(ip) .. " fue eliminado.")
	end
	...
	if(ban and doRemovePlayerBanishment(guid, PLAYERBAN_LOCK)) then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "El bloqueo de nombre de " .. param .. " fue eliminado.")
	end
