...
function onSay(cid, words, param, channel)
	...
	if(not guid) then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Jugador no encontrado.")
		return true
	end

	local hid = getHouseFromPos(getCreaturePosition(cid))
	if(not hid) then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "No estás en una casa.")
		return true
	end
