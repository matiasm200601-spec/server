local function ChangeBalls(cid, param, cancelequal)
	...
	for a = 1, #balls do
		local item = balls[a]
		if not getItemAttribute(item.uid, "ballorder") then return doPlayerSendTextMessage(cid, 27, "¡Actualiza la barra de tu Pokémon!") end --alterado v1.6
...
function onSay(cid, words, param)
	...
	if words == "/goaction" then
        ...
        if #getCreatureSummons(cid) >= 1 then
			if not getItemAttribute(getPlayerSlotItem(cid, 8).uid, "ballorder") then return doPlayerSendTextMessage(cid, 27, "¡Actualiza la barra de tu Pokémon!") end 
            --alterado v1.6
