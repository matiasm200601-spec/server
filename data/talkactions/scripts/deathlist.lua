...
function onSay(cid, words, param, channel)
	...
	if(target:getID() == -1) then
		doPlayerSendCancel(cid, "No existe un jugador con ese nombre.")
		return true
	end
	...
	if(deaths:getID() ~= -1) then
		repeat
			local killers = db.getResult("SELECT environment_killers.name AS monster_name, jugadores.name AS player_name FROM killers LEFT JOIN environment_killers ON killers.id = environment_killers.kill_id LEFT JOIN player_killers ON killers.id = player_killers.kill_id LEFT JOIN players ON jugadores.id = player_killers.player_id WHERE killers.death_id = " .. deaths:getDataInt("id") .. " ORDER BY killers.final_hit DESC, killers.id ASC" .. config.limit)
	...
	doPlayerPopupFYI(cid, "Lista de muertes del jugador: " .. targetName .. ".\n\n" .. str)
	return true
end
