...
for s = 1, #storages do
    if getPlayerStorageValue(cid, storages[s]) >= 1 then
       return doPlayerSendTextMessage(cid, 20, "No puedes hacer eso mientras vuelas, montas, navegas, buceas o usas una bicicleta.") 
    end
end
...
if (getCreatureName(mysum) == "Ditto") and getItemAttribute(thisball.uid, "ehditto") ~= -1 and isMonster(item2.uid) and pokes[getCreatureName(item2.uid)] then
    ...
    if getCreatureName(item2.uid) == "Ditto" or getCreatureName(item2.uid) == "Shiny Ditto" then    
        doPlayerSendCancel(cid, "Tu ditto can't transform into another ditto.")
        markPos(mysum, unfix)
    return true
    end
            if isInArray(proibidossempre, getCreatureName(item2.uid)) then    --Checa a lista
        doPlayerSendCancel(cid, "Tu ditto can't transform into that pokemon.")
        markPos(mysum, unfix)
    return true
    end
    ...
    if getCreatureName(item2.uid) == getPlayerStorageValue(mysum, 1010) then
        doPlayerSendCancel(cid, "Tu ditto is already transformed into that pokemon.")
        markPos(mysum, unfix)
    return true
    end
    markPos(mysum, unfix)
...
if isMonster(item2.uid) and getCreatureMaster(item2.uid) == cid then
	...
	doPlayerSendCancel(cid, "Tu pokemon can't use flash.")
	return true
	end
	...
	if cd > 0 then
	doPlayerSendCancel(cid, "Tu pokemon is too tired to use flash. Cooldown: ("..getStringmytempo(cd)..")")
	return true
	end
...
if isCreature(item2.uid) and ehMonstro(item2.uid) and isInArray(skills["control mind"], getCreatureName(mysum)) and item2.uid ~= mysum then
   ...
   if isSleeping(mysum) then
      return doPlayerSendCancel(cid, "Tu pokemon is sleeping...zZzZ")
   end

   if getTileInfo(getThingPos(cid)).protection or getTileInfo(getThingPos(getCreatureSummons(cid)[1])).protection then
      return doPlayerSendCancel(cid, "Tú o tu pokemon are in Pz zone!")
   end
   ...
   if cd > 0 then
      doPlayerSendCancel(cid, "Debes esperar "..cd.." segs to use Control Mind again!")
      return true
   end
   
   if getPokemonLevel(item2.uid) >= getPokemonLevel(mysum) then
      return doPlayerSendCancel(cid, "Tu pokemon can't control this mind!")
   end
