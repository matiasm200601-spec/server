function onJoinChannel(cid, channelId, users, isTv)
	...
	if channelId >= 13 and channelId <= 18 then
       ...
       if not isCreature(sid) then
	      doPlayerSendTextMessage(cid, 20, "¡El otro jugador no está conectado! Duelo cancelado.")
	   ...
	   if #pokes1 < (channelId-12) or #pokes2 < (channelId-12) then
	      ...
	      doPlayerSendTextMessage(cid, 20, "Tú o tu "..tpw.." no tienen esa cantidad de Pokémon en sus mochilas. Duelo cancelado.")
	      doPlayerSetVocation(cid, 1)  
	      for i = 1, #storagesDuel do
              setPlayerStorageValue(cid, storagesDuel[i], -1)
          end
...
function onTextEdit(cid, item, newText)
	...
	if item.itemid == 12330 then
		...
		if getPlayerStorageValue(cid, 99284) >= 1 then
			doPlayerSendCancel(cid, "¡Ya estás al aire!")
		return false
		end
		...
		if string.len(newText) <= 0 then
			doPlayerSendCancel(cid, "Tu canal se mostrará como \""..getCreatureName(cid).."'s TV Channel\".")
		elseif string.len(newText) > 25 then
			doPlayerSendCancel(cid, "El nombre de tu canal no puede tener más de 25 caracteres.")
		return false
		else
			channelName = newText
		end
