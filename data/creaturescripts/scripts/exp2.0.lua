...
if isPlayer(attacker) then
	...
	if #getCreatureSummons(attacker) >= 1 and not isInArray({POISONEDDAMAGE, BURNEDDAMAGE}, combat) then
		doPlayerSendTextMessage(attacker, MESSAGE_STATUS_DEFAULT, "Tu "..getPokeName(getCreatureSummons(attacker)[1]).." causó "..valor.." de daño a "..getSomeoneDescription(cid)..".")
	end
    ...
    if valor >= getCreatureHealth(cid) then
       ...
       if getPlayerStorageValue(cid, 22545) == 1 then
          if getGlobalStorageValue(22550) == 1 then
             doPlayerSendTextMessage(cid, 20, "¡Eres el último superviviente de la Arena Dorada! ¡Toma tu recompensa!")
       ...
       if getPlayerStorageValue(cid, 98796) >= 1 then
          ...
          doPlayerSendTextMessage(cid, 27, "Moriste en la zona safari. ¡Mejor suerte la próxima vez!")
          return false --alterado v1.8
       end
...
--------------------------------------------
    ...
    if isSummon(attacker) then
		...
		if isSummon(cid) and valor ~= 0 then
			addEvent(sendPlayerDmgMsg, 5, getCreatureMaster(cid), "Tu "..getCreatureName(cid).." lost "..valor.." hitpoints due to an attack from "..getSomeoneDescription(attacker)..".")
		end
